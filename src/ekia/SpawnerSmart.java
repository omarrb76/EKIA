...
public class SpawnerSmart extends GameObject {

    public SpawnerSmart(float x, float y, ID id, Handler handler, Color color, int ancho, int alto) {
        ...
        for (int i = 0; i < handler.object.size(); i++) {
            ...
            if (tempObject instanceof Jugador) {
                if (this.getBounds().intersects(tempObject.getBounds())) {
                    Sound clip = new Sound("res/Sonidos/SpawnBalasSmart.mp3");
                    clip.play();
                    ((Jugador) tempObject).setBalasSmart(3); // Le da 3 balas smart
